...
/*   Updated: 2022/02/03 14:48:27 by jumanner         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	get_allocations(char **prefix, char **str, long long value)
{
	*prefix = ft_strnew(2);
	ft_strcpy(*prefix, "0x");
	*str = ft_itoa_llong_base(value, 16);
	if (!(*prefix) || !(*str))
	{
		free(prefix);
		free(str);
...
static int	hex_writer(long long value, t_flags f, char *(*mod)(char *))
{
	char	*str;
	char	*prefix;
	int		len;
...
int	hex_handler(va_list args, t_flags f, char *(*mod)(char *))
{
	if (ft_strequ(f.length, "h"))
		return (hex_writer((unsigned short) va_arg(args, int), f, mod));
	if (ft_strequ(f.length, "hh"))
		return (hex_writer((unsigned char) va_arg(args, int), f, mod));
	if (ft_strequ(f.length, "l"))
		return (hex_writer(va_arg(args, unsigned long), f, mod));
	if (ft_strequ(f.length, "ll"))
		return (hex_writer(va_arg(args, unsigned long long), f, mod));
	if (ft_strequ(f.length, "j"))
		return (hex_writer(va_arg(args, uintmax_t), f, mod));
	return (hex_writer(va_arg(args, unsigned int), f, mod));
}
