...
static int	bin_writer(long long value, t_flags f, char *(*mod)(char *))
{
	char	*str;
	char	*prefix;
	int		len;
...
int	bin_handler(va_list args, t_flags f, char *(*mod)(char *))
{
	if (ft_strequ(f.length, "h"))
		return (bin_writer((unsigned short) va_arg(args, int), f, mod));
	if (ft_strequ(f.length, "hh"))
		return (bin_writer((unsigned char) va_arg(args, int), f, mod));
	if (ft_strequ(f.length, "l"))
		return (bin_writer(va_arg(args, unsigned long), f, mod));
	if (ft_strequ(f.length, "ll"))
		return (bin_writer(va_arg(args, unsigned long long), f, mod));
	if (ft_strequ(f.length, "j"))
		return (bin_writer(va_arg(args, uintmax_t), f, mod));
	return (bin_writer(va_arg(args, unsigned int), f, mod));
}
